...
#include "/home/avees/data_split/src/data-domain-split/avt_vimba_camera/include/avt_vimba_camera/mono_camera_node.hpp"
...
void forward_network_gpu(network net, network_state state)
{
	...
	if( i == net.n - 2)
	  {     int output_size = l.outputs * l.batch;
		float *host_output = (float*)malloc(output_size * sizeof(float));
		// cudaMemcpy 호출
		cudaError_t err = cudaMemcpy(host_output, l.output_gpu, output_size * sizeof(float), cudaMemcpyDeviceToHost);
		if (err != cudaSuccess) {
		...
		yolo_data.data.resize(output_size);
		for (int j=0;j<output_size; j++)
		{ yolo_data.data[j] = host_output[j];}
